...
async function main() {
  try {
    ...
    console.log('First 3:', recentDocs.slice(0, 3).map(p => p.split('/').pop()))
    ...
    console.log('First 3:', highResImages.slice(0, 3).map(p => p.split('/').pop()))

    // 4. Find audio files with specific properties
    console.log('\n4. Finding high-quality audio files:')
    const highQualityAudio = await mdfind('kMDItemAudioSampleRate > 44100 && kMDItemAudioBitRate > 320000')
    console.log(`Found ${highQualityAudio.length} high-quality audio files`)
    console.log('First 3:', highQualityAudio.slice(0, 3).map(p => p.split('/').pop()))
    ...
    console.log('First 3:', authorDocs.slice(0, 3).map(p => p.split('/').pop()))
    ...
    console.log('First 3:', cameraPhotos.slice(0, 3).map(p => p.split('/').pop()))
